...
function buildLainnyaFromMedications(
  ...
  return {
    ...
    askep,
    observasi:
      'Pantau tanda vital per 4-6 jam. Monitor respons terapi. Perhatikan perubahan kondisi klinis seperti penurunan kesadaran, nyeri memberat, atau gejala baru.',
...
function buildAskep(keluhan: string, dxLabel: string): string {
  ...
  if (/batuk|sesak|nafas|pilek/.test(keluhan)) {
    lines.push(
      'Gangguan pola napas: observasi frekuensi dan kedalaman napas, posisikan semi-fowler.'
    );
  }
  if (/nyeri|sakit/.test(keluhan)) {
    lines.push(
      'Nyeri akut: kaji skala nyeri, berikan posisi nyaman, kolaborasi analgetik sesuai advis.'
    );
  }
  if (/mual|muntah|diare|perut/.test(keluhan)) {
    lines.push(
      'Gangguan keseimbangan cairan: monitor intake-output, anjurkan cairan oral adekuat.'
    );
  }
  ...
  lines.push(
    'Defisit pengetahuan: berikan edukasi tentang penyakit, pengobatan, dan pencegahan kambuh.'
  );
  return lines.join(' ');
}
